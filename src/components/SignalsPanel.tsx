import type { SymbolIndicatorsResponse } from '@/lib/types';

export function SignalsPanel({ data }: { data: SymbolIndicatorsResponse }) {
  const rs = data?.rulescore;
  const ind = data?.indicators;
  return (
    <div className="text-sm space-y-1">
      <div>Bias: <b>{rs?.bias}</b> · Confidence: <b>{rs?.confidence}</b> · Score: <b>{rs?.score}</b></div>
      <div>RSI(14): {ind?.rsi14 != null ? ind.rsi14.toFixed(1) : '—'}</div>
